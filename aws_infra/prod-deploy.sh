...
echo "Prod deploy start ` date "+%Y/%m/%d %H:%M:%S"`"

echo "Step1 switch eks ` date "+%Y/%m/%d %H:%M:%S"`"
aws eks update-kubeconfig --name renft-eks --region ap-northeast-1 --role-arn  arn:aws:iam::629667697317:role/renft-eks-bastion
...
docker tag renft/api:latest 629667697317.dkr.ecr.ap-northeast-1.amazonaws.com/renft-api:latest
...
docker tag renft/frontend:latest 629667697317.dkr.ecr.ap-northeast-1.amazonaws.com/renft-frontend:latest
...
docker push 629667697317.dkr.ecr.ap-northeast-1.amazonaws.com/renft-api:latest
...
docker push 629667697317.dkr.ecr.ap-northeast-1.amazonaws.com/renft-frontend:latest
...
if [ $cmdstatus -ne 0 ]; then
  ...
  kubectl apply -f ./yaml/prod/api.yaml
...
if [ $cmdstatus -ne 0 ]; then
  echo "try to apply start"
  kubectl apply -f ./yaml/prod/frontend.yaml
  echo "try to apply end"
fi
echo "Step9 rollout frontend end ` date "+%Y/%m/%d %H:%M:%S"`"

echo "Prod deploy end ` date "+%Y/%m/%d %H:%M:%S"`"